...
const Index = () => {
   ...
   const FetchData =async(sortdata)=>{
      try{
         const {data}=await getBucket(sortdata);
         console.log(data)
         setBucketdta(data.records)
        }catch(error){
         console.log(error);
        }
   }
    ...
    <div className="flex w-full bg-gray-100 text-black">
     ...
     <div className='w-9/12 mx-auto pt-8 h-screen'>
        <div className='flex items-center justify-between'>
            <h1 className=' text-lg font-semibold'>Make Bucket</h1>
      ...
      {bucketdata?.map(Ele=>{
         return (
            <div className='flex w-full mt-4 justify-between px-4 p-3 bg-white border rounded' key={Ele.id}>
         <div>
         ...
         <Link href={`/bucket/makebucket/${Ele.id}`}>
         <div>
            <button className=' text-prime-red font-semibold border-prime-red rounded py-1 border-[1.5px] w-32'>{Ele.filled[0]=='0'?'Make Bucket':Ele.filled}</button>
         </div>
         </Link>
        </div>
         )
